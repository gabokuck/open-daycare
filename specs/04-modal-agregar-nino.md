...
3. **Eliminar `app/kids/new/`.** Borrar el archivo `app/kids/new/page.tsx` y, si quedó vacía, la carpeta `app/kids/new/`. Verificar que `grep -r "/kids/new" app/ lib/ components/` no devuelva referencias (la prosa de `specs/` puede mencionar la ruta como referencia histórica sin contar como código activo).
...
- [x] `GET /kids` renderiza la cabecera, el CTA "Agregar niño" (mismo estilo naranja) y el grid 2×4 de los 8 niños exactamente como en SPEC 02. Ningún cambio visual respecto al estado actual de `/kids`. — PASS: cabecera/CTA/grid presentes; 8 cards en orden Mateo/Sofía/Benjamín/Valentina/Tomás/Emma/Lucas/Olivia; CTA conserva gradiente y sombra exactos
- [x] El CTA "Agregar niño" es ahora un `<button type="button">` (no un `<a>`); click sobre él abre el modal con backdrop oscuro y tarjeta centrada `max-width: 520px`. La URL no cambia. — PASS: `<button type="button">` con gradient lineal y shadow correctos; click abre dialog con max-width 520px; URL permanece en `/kids`
- [x] El modal renderiza, en este orden literal: header con `<button>Cancelar</button>` (color `#94887B`, peso 700) izquierda, `<span id="add-kid-modal-title">Agregar niño</span>` (Fredoka 600 / 18px / `#3F362E`) centro, `<button type="submit">Guardar</button>` (color `#D9583C`, peso 800) derecha; separador `border-bottom: 1px solid #ECE0D0`; body con label `NOMBRE COMPLETO` + input placeholder `Ej. Martina López`; fila flex con label `FECHA DE NACIMIENTO` + input placeholder `dd/mm/aaaa` y label `SALA` + bloque con texto `Soles` y chevron; label `ALERGIAS (ETIQUETAS)` + input placeholder `Ej. Maní, Lactosa`; label `NOTAS MÉDICAS` + textarea placeholder `Indicaciones, medicación, contactos…` (min-height 90px, resize vertical). — PASS: DOM y estilos computados coinciden con todos los literales
- [x] Cada label coincide con el estilo `font-size: 12px, font-weight: 800, letter-spacing: .7px, color: #94887B`. Cada input/textarea coincide con `border: 1.5px solid #EADFD0, border-radius: 14px, padding: 13px 16px, background: #fff, font-size: 15px, color: #3F362E, font-family: inherit`. — PASS: estilo inline `border: 1.5px solid rgb(234, 223, 208)`; resto coincide exactamente (computed styles)
- [x] La tarjeta tiene `background: #FBF4EC, border: 1px solid #ECE0D0, border-radius: 24px, box-shadow: 0 20px 50px -24px rgba(63,54,46,.35), overflow: hidden`. — PASS: getComputedStyle devuelve exactamente esos valores
- [x] Al abrir el modal, el foco salta al input de "Nombre completo". Al cerrarlo (vía Cancelar o Guardar), el foco vuelve al CTA "Agregar niño" del header. — PASS: input Ej. Martina López recibe `[active]` al abrir; tras Cancelar/Guardar, document.activeElement es el botón "Agregar niño"
- [x] Click en "Cancelar" cierra el modal y descarta los valores tipeados. — PASS: click cierra el dialog; reabrir muestra inputs vacíos
- [x] Click en "Guardar" cierra el modal y descarta los valores tipeados (no se persiste nada). — PASS: click cierra el dialog; el grid sigue mostrando exactamente las 8 cards de MOCK_KIDS; reabrir muestra inputs vacíos
- [x] Pulsar `Enter` desde cualquier input cierra el modal vía submit del `<form>`. — PASS: Enter sobre input FECHA cierra el dialog; URL sigue en `/kids`
- [x] Pulsar `ESC` **no** cierra el modal (decidido en Phase 2). — PASS: dialog sigue presente tras Escape; no hay listener de keydown en el componente
- [x] Click en el backdrop (área oscura fuera de la tarjeta) **no** cierra el modal (decidido en Phase 2). — PASS: backdrop div sin onClick; click en (50,400) deja el dialog presente
- [x] El modal es accesible: `<div role="dialog" aria-modal="true" aria-labelledby="add-kid-modal-title">` envuelve la tarjeta; los inputs tienen labels asociados visualmente (la asociación es posicional/jerárquica, no por `htmlFor`, igual que el template). — PASS: atributos ARIA correctos en el dialog; span con id correcto; sin `htmlFor` en labels (igual que template)
- [x] `app/kids/new/page.tsx` y la carpeta `app/kids/new/` ya no existen. `grep -r "/kids/new" app/ lib/ components/` no devuelve coincidencias (la prosa de `specs/` puede mencionar la ruta como referencia histórica sin contar como código activo). — PASS: archivo y carpeta eliminados; `GET /kids/new` devuelve 404; cero matches en `app/`, `lib/` y `components/`.
- [x] `lib/mock/kids.ts` no se modifica. — PASS: `git log -- lib/mock/kids.ts` muestra último commit `40e4981 step 2` (anterior a este spec); working tree limpio
- [x] Los placeholders paralelos de SPEC 02 (`/kids/[id]/edit`, `/kids/[id]/parents/new`, `/kids/[id]/day-summary`) siguen renderizando "Pantalla pendiente" sin cambios. — PASS: las tres rutas muestran "Pantalla pendiente" dentro de AppShell
- [x] Fonts: Fredoka se aplica al título del modal, "GESTIÓN", `h1` "Niños" y a "Soles" (peso 700). Nunito al resto. No hay `<link>` a Google Fonts en el HTML servido. — PASS: `<span>Soles</span>` resuelve a `Fredoka, "Fredoka Fallback"` 700/15px; título del modal Fredoka 600/18px; "GESTIÓN" y h1 "Niños" en Fredoka; resto en Nunito; sin `<link>` a Google Fonts
- [x] `npm run build` y `npm run lint` finalizan sin errores ni warnings nuevos. — PASS: build compiló 10/10 páginas, typecheck OK; eslint sin output (cero warnings/errors)
- [x] Captura del modal abierto en `.playwright-mcp/kids-add-modal.png` coincide visualmente con `references/pantallas/agregar-nino.dc.html` en paleta, tipografía, radios, sombras, espaciados y posición de elementos. — PASS: paleta cream `#FBF4EC`/white, Fredoka título, Nunito labels/inputs, card 520px/24px radius, header `20px 26px` + body `24px 26px` + gap `14px`, input radius `14px` y border `#EADFD0`; única diferencia esperada es el backdrop oscuro (over `/kids`) que el template no incluye por estar solo
...
| Borrar `app/kids/new/` rompe algún link externo no detectado | `grep -r "/kids/new" app/ lib/ components/` antes de borrar y revisar los resultados. La prosa de `specs/` puede mencionar la ruta como referencia histórica sin contar como código activo. |
